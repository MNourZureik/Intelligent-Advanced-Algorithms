...
public class Computer {
    ...
    /*
     * Initial Minimax algorithm.
     */

    public static List<Directions> minimaxDecision(GameBoard currentBoard, int depth) {
        long startTime = System.currentTimeMillis();
        // Wrapper object to hold the best path
        List<Directions> bestPath = new ArrayList<>();

        // Initialize the best value
        int bestValue = Integer.MIN_VALUE;

        // Explore all possible moves from the current state
        for (Map.Entry<Directions, GameBoard> move : nextState(currentBoard)) {
            List<Directions> path = new ArrayList<>();
            int value = minimax(move.getValue(), depth - 1, false, path); // Minimize next
            if (value > bestValue) {
                bestValue = value;
                bestPath = new ArrayList<>(path); // Update the best path
                bestPath.addFirst(move.getKey()); // Prepend the current move
            }
        }

        return ReturnMinMaxGoalPath(bestPath , startTime); // Return the path of the best moves
    }

    private static int minimax(GameBoard state, int depth, boolean isMaximizing, List<Directions> path) {
        // Base Case: Terminal state (goal reached or depth limit)
        if (depth == 0 || state.getGoalsMap().isEmpty()) {
            return Heuristic.ManhattanUncharacteristic(state); // Heuristic evaluation
        }
        ...
        if (isMaximizing) {
            ...
            for (Map.Entry<Directions, GameBoard> move : nextState(state)) {
                List<Directions> tempPath = new ArrayList<>();
                int eval = minimax(move.getValue(), depth - 1, false, tempPath);
                if (eval > maxEval) {
                    maxEval = eval;
                    bestPath = new ArrayList<>(tempPath); // Update the best path
                    bestPath.add(0, move.getKey()); // Prepend the current move
                }
            }
            path.addAll(bestPath); // Update the current path with the best path
            return maxEval;
            ...
            for (Map.Entry<Directions, GameBoard> move : nextState(state)) {
                List<Directions> tempPath = new ArrayList<>();
                int eval = minimax(move.getValue(), depth - 1, true, tempPath);
                if (eval < minEval) {
                    minEval = eval;
                    bestPath = new ArrayList<>(tempPath); // Update the best path
                    bestPath.addFirst(move.getKey()); // Prepend the current move
    ...
    public static List<Directions> play(String type, int index) {
        ...
        long memoryUsed = HelpingFunctions.measureDataStorageUsage(() -> {
            ...
            } else if ("min-max".equalsIgnoreCase(type)){
                result.addAll(Objects.requireNonNull(minimaxDecision(gameBoard , 10)));
            }
        });
    ...
    static List<Directions> ReturnMinMaxGoalPath(List<Directions> bestPath , long startTime){
        try (PrintWriter logWriter = new PrintWriter(new FileWriter("algorithm_output.log", true))) {

            logWriter.println("Algorithm: Minimax");
            logWriter.println("Path Length: " + bestPath.size());
            logWriter.println("Directions: " + bestPath);
            logWriter.println("Time: " + (System.currentTimeMillis() - startTime / 1000.0) + " second");
        }
        catch (IOException e) {
            e.printStackTrace();
        }
